...
if(is.null(file1)){return(NULL)}
  ...
  d<- nchar(gsub("\t", "", a[1]))
  if(b<c&b<d){
    #r <- read.table(path,sep=";",stringsAsFactors = FALSE,nrows=1,na.strings = c("NA",""))
    r <- read.table(path,sep=";",stringsAsFactors = FALSE,nrows=1)
  ...
  #r1<-read.table(path,sep=SEP,stringsAsFactors = FALSE,nrows=1,skip = 1,na.strings = c("NA",""))
  r1<-read.table(path,sep=SEP,stringsAsFactors = FALSE,nrows=1,skip = 1)
  aciertos <- 0
  for(i in 1: ncol(r)){
...
data <- read.table(path,sep=SEP,header=HEADER)
